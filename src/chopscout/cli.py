...
def parser() -> argparse.ArgumentParser:
    root = argparse.ArgumentParser(prog="chopscout", description="Prepare breaks for standalone MPC workflows.")
    sub = root.add_subparsers(dest="command", required=True)
    analyze = sub.add_parser("analyze", help="Analyze an audio file")
    analyze.add_argument("input")
    export = sub.add_parser("export", help="Analyze and export WAV/MIDI and, for 16 slices, an MPC 3.9.0 XPJ project")
    export.add_argument("input")
    export.add_argument("--mode", default="transient", choices=["transient", "equal8", "equal16", "equal32", "equal48", "equal64", "beat", "eighth", "sixteenth", "hybrid", "manual"])
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "analyze":
            ...
            print(json.dumps({
                "audio": asdict(result.audio), "detected_bpm": result.detected_bpm,
                "half_time_bpm": result.half_time_bpm, "double_time_bpm": result.double_time_bpm,
                "tempo_confidence": result.tempo_confidence, "downbeat": result.downbeat,
                "estimated_bars": result.estimated_bars, "transients": len(result.onset_times),
                "warnings": result.warnings,
            }, indent=2))
            return 0
        if args.bpm:
            project.analysis.selected_bpm = args.bpm
        settings = ExportSettings(mode=args.mode, starting_note=args.starting_note,
                                  bars=args.bars or project.analysis.estimated_bars,
                                  bpm=args.bpm or project.analysis.selected_bpm,
                                  overwrite=args.overwrite,
                                  pad_count=args.pad_count or len(project.markers))
