...
| `test_autosave.py` | Autosave round trip and metadata, corrupt/hostile slot rejection, atomic overwrite, dirty-only writes, clearing on save and clean exit, and the startup recover/discard prompt (requires `QT_QPA_PLATFORM=offscreen`) |
